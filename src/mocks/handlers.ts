...
export const handlers = [
  ...
  http.get('/api/participations', async ({ request }) => {
    ...
    const tenantId = url.searchParams.get('tenantId');
    const db = getDb();
    const joined = db.participations
      .filter((p) => (!clientId || p.clientId === clientId) && (!tenantId || p.tenantId === tenantId))
      .map((p) => {
        const promotion = db.promotions.find((promo) => promo.id === p.promotionId);
        if (promotion && isExpired(promotion.expiresAt) && p.status === 'ACTIVE') {
          p.status = 'EXPIRED_BLOCKED';
